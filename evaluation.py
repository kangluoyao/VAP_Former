...
def evaluate_model(device_in, uuid, ld_helper):
    ...
    fold = 0
    path = "../weights/{}/".format(task_str) + uuid + "/best_weight"
    print(path)
    ...
    accuracy, sensitivity, specificity, precision, F1, roc_auc, you_max, you_thresh, auc = [*metrics]
...
def get_roc_auc(model_in, test_dl, figure=False, path=None, fold=1):
    ...
    for t in range(0, 11, 1):
        ...
        thresh = t/10
        acc, sens, spec,precision, F1, auc = get_metrics(model_in, test_dl, thresh)
        
        tpr.append(sens)
        fpr.append(1 - spec)
        print("threshold: ", thresh, 'acc:', acc, 'sens', sens, 'spe', spec, 'precision', precision, 'F1',F1, 'auc', auc)
        total_auc += auc
    ...
    try:
        ...
        print(e)
    metrics = [opt_acc, opt_sens, opt_spec, opt_precision, opt_F1, roc_auc, youdens_s_max, optimal_thresh, total_auc/11]
